...
- `make linux-test` — runs `swift test` inside the Linux dev container (requires the `container` CLI from apple/container). Passes `--cap-add CAP_SYS_ADMIN` so the network-namespace spawn tests can create a namespace with `unshare --net`; without it those tests report as skipped. The dev image tag is keyed on a hash of `images/linux-dev/Dockerfile` and the files it copies in, so editing the recipe forces exactly one rebuild instead of silently reusing a stale image.
...
- `make linux-integration` — runs the cross-platform integration suite against a real cloud-hypervisor VM inside the dev container (nested virt via apple/container's `--virtualization`). Requires a KVM-capable kernel at `kernel/vmlinux-arm64` (or `kernel/vmlinuz-x86_64` on x86_64 hosts) — build via `make -C kernel`; the kata-fetched kernel doesn't include KVM. Also requires `make fetch-cloud-hypervisor` and `make linux-build` to have been run first. Linux runs only the cross-platform subset (`process true`/`false`/`echo hi`); the macOS suite is unchanged. Also passes `--cap-add CAP_SYS_ADMIN --cap-add CAP_NET_ADMIN` for the "CH process network namespace" test (namespace creation + the TUNSETIFF that makes the tap inside it); that test reports as skipped without them, or if `ip` is missing from the dev image.
...
- **Linux**: `CHVirtualMachineManager` / `CHVirtualMachineInstance` (`CH*` files plus `CHProcess`, `VirtiofsdProcess`, `Vsock+Linux`, all `#if os(Linux)`). One `cloud-hypervisor` subprocess per VM, REST-on-UDS control plane via the standalone [`CloudHypervisor`](./Sources/CloudHypervisor) Swift package, virtio-blk / virtio-fs (one `virtiofsd` per share) / TAP / vsock for the data plane. Same `Vminitd` guest contract as VZ — only the host-side VMM differs.
  ...
  **Network namespaces.** `CHVirtualMachineManager(networkNamespace:)` spawns each VM's cloud-hypervisor process inside an existing network namespace (`/proc/<pid>/ns/net` or a bind mount), which is what a CNI-prepared pod netns needs: CH resolves `NetConfig.tap` by name in its own namespace, and `TUNSETIFF` on a name that isn't there *creates* an unattached tap instead of failing — a silent loss of connectivity. The `setns` happens in the child between `fork` and `execve` (`Sources/CShim/exec_command.c`, surfaced as `Command.Attrs.networkNamespace`), so the host process never moves. `virtiofsd` is deliberately left in the caller's namespace: it only speaks over unix sockets by path.
